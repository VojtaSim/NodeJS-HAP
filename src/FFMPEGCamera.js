...
function FFMPEG(hap, cameraConfig, log, videoProcessor) {
  ...
  this.maxFps = ffmpegOpt.maxFPS || 10;
  ...
  this.source = ffmpegOpt.source;
  this.stillImageSource = ffmpegOpt.stillImageSource;
...
FFMPEG.prototype.handleSnapshotRequest = function(request, callback) {
  ...
  var imageSource = this.stillImageSource !== undefined ? this.stillImageSource : this.source;
  ...
  if (sessionID) {
    ...
    if (requestType == "start") {
      ...
      if (sessionInfo) {
        ...
        let ffmpegCommand = this.source + ' -map 0:0' +
          ' -vcodec ' + vcodec +
          ' -pix_fmt yuv420p' +
          ' -r ' + fps +
          ' -f rawvideo' +
